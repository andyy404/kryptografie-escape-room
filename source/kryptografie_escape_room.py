...
class Door():
    ...
    def return_drawing(self):
        ...
        self.lines = (
            (w/50*11 + self.pos5, h/50*28),
            (w/50*12 + self.pos5, h/50*28),
            (w/50*12 + self.pos5, h/50*32),
            (w/50*11 + self.pos5, h/50*32),
            (w/50*11 + self.pos5, h/50*28)
        )
...
room = None # contains the current room object # FIXME
...
def menu(): # shows the main menu screen
    ...
    if mbdown and button_menu_play.collidepoint((mx, my)): # if a mouse button is clicked while the cursor is over a button, the scene will switch to the correct one
        ...
        paused = False
        timer_running = True
        rooms_list = [Room1(), Room2(), Room3()] # ADDHERE all the rooms
        room = rooms_list[0]
